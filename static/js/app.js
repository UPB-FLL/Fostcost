...
async function loadDashboard() {
  ...
  document.getElementById('kpi-row').innerHTML = `
    <div class="kpi-card"><div class="kpi-val">${items.length}</div><div class="kpi-lbl">Products</div></div>
    <div class="kpi-card"><div class="kpi-val">${ingredients.length}</div><div class="kpi-lbl">Ingredients</div></div>
    <div class="kpi-card"><div class="kpi-val">${target}%</div><div class="kpi-lbl">Target %</div></div>
  `;
}
...
async function processImport() {
  ...
  try {
    ...
    if (!Array.isArray(data)) throw new Error('JSON must be an array of ingredients');
...
async function importSquareMenu() {
  if (!confirm('Import all Square menu items as products?')) return;
